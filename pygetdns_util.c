...
extensions_to_getdnsdict(PyDictObject *pydict)
{
    ...
    Py_ssize_t pos = 0;
    PyObject *key, *value;
    struct getdns_bindata *list_bindata;
    char *tmp_string;
    int  tmp_int;
    char *tmp_key;
    ...
    while (PyDict_Next(pydict, &pos, &key, &value))  {
        list_bindata = (struct bindata *)malloc(sizeof(struct getdns_bindata));
        tmp_key = PyString_AsString(PyObject_Str(key));
        if (!strncmp(tmp_key, "return_both_v4_and_v6", strlen("return_both_v4_and_v6")))  {
            if (!PyInt_Check(value))  {
                PyErr_SetString(getdns_error, "Type error in return_both_v4_and_v6 value");
                return NULL;
            }
